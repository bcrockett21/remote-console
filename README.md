...
- a relay server with explicit auth-mode configuration, a public `/api/config` capability summary, shared-key protection for current agent traffic, Entra-backed viewer validation for Teams SSO mode, and file-backed or env-backed session-level viewer bindings
